...
/**
 * @title MerkleFeeCollectorUpgradeable
 * @notice Upgradeable version of MerkleFeeCollector
 * 
 * UPGRADEABLE DESIGN:
 ...
 */
contract MerkleFeeCollectorUpgradeable is ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "MerkleFeeCollector: caller is not owner");
        _;
    }

    function initialize(address _owner) external {
        require(!_initialized, "MerkleFeeCollector: already initialized");
        require(_owner != address(0), "MerkleFeeCollector: owner is zero address");
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "MerkleFeeCollector: new owner is zero address");
