...
/**
 * @title EndowmentWalletUpgradeable
 * @notice Upgradeable version of EndowmentWallet
 * 
 * UPGRADEABLE DESIGN:
 ...
 */
contract EndowmentWalletUpgradeable is ReentrancyGuard {
    ...
    modifier onlyOwner() {
        require(msg.sender == owner, "EndowmentWallet: caller is not owner");
        _;
    }

    function initialize(address _owner) external {
        require(!_initialized, "EndowmentWallet: already initialized");
        require(_owner != address(0), "EndowmentWallet: owner is zero address");
    ...
    function transferOwnership(address newOwner) external onlyOwner {
        require(newOwner != address(0), "EndowmentWallet: new owner is zero address");
